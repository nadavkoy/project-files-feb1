    ...
    def server_verification(self):
        ...
        if server_response.startswith(SUCCESSFUL_ENTRY):  # if entry was successful:
            ...
            if self.entrance_type == 'login':
                ...
                self.password_validation_text.SetLabel('USERNAME TAKEN. \n  please try again.')
...
class ChatPanel(wx.Panel):
    ...
    def __init__(self, parent_frame):
        ...
        t2 = threading.Thread(target=self.get_songs, args=())
        t2.start()
    ...
    """def get_songs(self):
        while True:
            song = ''
            name_and_length = self.parent_frame.song_client.client_socket.recv(1024)
            name_and_length = name_and_length.split(SEPARATOR)
            song_path = self.parent_frame.get_username() + "\\" + name_and_length[0]
            print song_path
            file_length = int(name_and_length[1])
            self.parent_frame.song_client.client_socket.send('file length accepted successfully')
            print 'file size' + str(file_length)

            while file_length > 0:
                packet = self.parent_frame.song_client.client_socket.recv(1024 * 40)
                song += packet
                file_length = file_length - len(packet)

            self.parent_frame.song_client.client_socket.send('song received successfully.')

            #new_file_name = 'new_song' + str(random.randint(1, 100)) + '.mp3'
            #print new_file_name
            new_file = open(song_path, 'wb')
            new_file.write(song)
            new_file.close()

            self.parent_frame.songs_to_play.append(song_path)
    ...
    def get_songs(self):
        while True:
            song = ''
            name = self.parent_frame.song_client.client_socket.recv(1024)
            song_path = self.parent_frame.get_username() + "\\" + name

            self.parent_frame.song_client.client_socket.send('file name accepted successfully')
            print 'file name: ' + name

            piece = ''
            while END_OF_SONG not in piece:
                piece = self.parent_frame.song_client.client_socket.recv(1024)
                song += piece

            song = song.split(END_OF_SONG)[0]
...
class Frame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.song_client = song_client.SongClient()  # creating the song transfer client.

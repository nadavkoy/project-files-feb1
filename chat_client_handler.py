...
class ClientHandler(threading.Thread):
    ...
    """def send_songs(self):
        ...
        for song in SONG_LIST:
            ...
            self.send_song(packets_list, file_size, song_name)

        def send_song(self, packets_list, file_size, song_name):
            self.song_socket.send(song_name + SONG_SEPARATOR + str(file_size))
    ...
    def send_songs(self):
        ...
        for song in SONG_LIST:
            ...
            self.song_socket.send(song_name)
            self.song_socket.recv(1024)

            while True:
                piece = file.read(1024)
                self.song_socket.send(piece)

                if piece == '':
                    break

            self.song_socket.send(END_OF_SONG)
